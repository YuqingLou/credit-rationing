...
newdata <- newdata %>%
  ...
  filter(group_n > 1)
# 检查一下新IV的数据特征
# summary(newdata$iv_region)
describe(newdata)

# Step2: 线性概率模型LPM（2SLS+RSE）
# 第一阶段用工具变量预测内生变量
fit_first_stage <- lm(digital_participation ~ iv_region + age + sex + education + 
                        marriage + health + prefer + income + size + rural, 
                      data = newdata) # 记得确保 data 是去除了那 11 个缺失值的最终数据
summary(fit_first_stage)
# 两阶段最小二乘（2SLS）
fit_2sls <- ivreg(
  operation ~ digital_participation + age + sex + education + marriage + 
    health + prefer + income + size + rural |
    iv_region + age + sex + education + marriage + 
    health + prefer + income + size + rural,
  data = newdata
)
# 查看第一阶段 F 统计量，并使用异方差稳健标准误进行检验
summary(fit_2sls, diagnostics = TRUE)
coeftest(fit_2sls, vcov = vcovHC(fit_2sls, type = "HC1"))
modelsummary(fit_2sls, 
             vcov = "robust",  # 关键参数：指定稳健标准误
             stars = TRUE,
             fmt = 4,
             title = "IV回归结果 (Robust SE)")
data_final_sample <- model.frame(fit_2sls)

# Step3: 广义联合回归模型GJRM补充检验
# 构建两阶段回归模型
eq_1 <- operation ~ digital_participation + age + sex + education + 
  marriage + health + prefer + income + size + rural
eq_2 <- digital_participation ~ iv_region + age + sex + education + 
  marriage + health + prefer + income + size + rural
# margins 参数：第一个是因变量分布(probit)，第二个是内生变量分布(normal)
# Model = "B" 表示 Bivariate (双变量联立)
fit_gjrm <- gjrm(list(eq_1, eq_2), data = data_final_sample, margins = c("probit", "N"), model = "B")
summary(fit_gjrm)
